...
export default function CheckTicket(ticket, userDetails) {
  ...
  const modify_ticket_html = (
    <Button
      ...
      colorScheme="blue"
      onClick={() =>
        navigateToModifyTicketPage(
          ticket.public_service_request_id,
          ticket.status
        )
      }
      width="13em"
  ...
  if (status === "tenant_ticket_created") {
    if (
      userDetails().type === "landlord" &&
      userDetails().email === ticket.landlord_email
    ) {
      return (
        <>
          <Checkbox
            id="quotationCheckbox"
  ...
  if (status === "landlord_ticket_approved") {
    if (
      ...
      if (
        ticket.quotation_required === "true" ||
        ticket.quotation_required === "1"
      ) {
        return (
          <Button
            variant="solid"
