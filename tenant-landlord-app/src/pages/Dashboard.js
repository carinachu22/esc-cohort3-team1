...
import {
  AiFillInfoCircle,
  AiFillClockCircle,
} from "react-icons/ai/index.esm.js";
...
export default function Dashboard() {
  ...
  const authenticate = () => {
    console.log("useIsAuthenticated returns ");
    console.log(useIsAuthenticated);
    console.log("authenticated() returns ", authenticated());
  ...
  console.log(
    "This is for react testing. useIsAuthenticate is ",
    useIsAuthenticated()
  );
  if (!authenticated()) {
    return <Navigate to="/" />;
        ...
        <Box maxW="7xl" mx={"auto"} pt={5} px={{ base: 2, sm: 12, md: 17 }}>
          ...
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={{ base: 5, lg: 8 }}>
            {StatsCard(
              "Total Tickets",
              tickets.length,
              <AiFillInfoCircle size={"3em"} color="lightblue" />
            )}
            {StatsCard(
              "Tickets Requiring Your Attention",
              needAction,
              <BiSolidError size={"3em"} color="orange" />
            )}
            {StatsCard(
              "Tickets Pending Your Landlord's Action",
              waitAction,
              <AiFillClockCircle size={"3em"} color="grey" />
            )}
          </SimpleGrid>
        </Box>
      </>
      ...
      <>
        ...
        <Box maxW="7xl" mx={"auto"} pt={5} px={{ base: 2, sm: 12, md: 17 }}>
          ...
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={{ base: 5, lg: 8 }}>
            ...
            {StatsCard(
              "Tickets Pending Your Tenant's Action",
              waitAction,
              <AiFillClockCircle size={"3em"} color="grey" />
            )}
          </SimpleGrid>
